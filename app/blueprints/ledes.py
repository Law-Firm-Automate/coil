...
def missing_ids(firm, invoices):
    """Human-readable list of the ids a LEDES export needs and does not have."""
    problems = []
...
def invoice_lines(firm, inv):
    """Yield one 24-field list per time or expense line on the invoice."""
    from ..models import TimeEntry
    lines = [l for l in inv.lines if l.kind in ("time", "expense")]
    ...
    for l in lines:
        n += 1
        src = source_for_line(l)
        is_time = l.kind == "time"
        if is_time:
            units = float(l.quantity or 0)
            unit_cost = int(l.unit_cents or 0)
            task = getattr(src, "task_code", "") if src else ""
            activity = getattr(src, "activity_code", "") if src else ""
            expense = ""
        else:
            units = 1.0
        ...
        gross = int(round(units * unit_cost))
        adjustment = int(l.amount_cents or 0) - gross  # split shares show up here so units x cost + adj = total
        user = getattr(src, "user", None) if isinstance(src, TimeEntry) else None
        tk_id = (user.initials or str(user.id)) if user else ""
        tk_name = user.name if user else ""
        tk_class = ROLE_TO_CLASSIFICATION.get(user.role if user else "", "OT") if user else ""
        yield [
            ...
            str(n), "F" if is_time else "E", f"{units:.2f}", _amt(adjustment), _amt(l.amount_cents),
...
def build_1998b(firm, invoices):
    """Return the full LEDES 1998B text: header, field row, then one record per fee/expense line."""
    out = [LEDES_HEADER, "|".join(LEDES_FIELDS) + "[]"]
    for inv in invoices:
        for row in invoice_lines(firm, inv):
            assert len(row) == 24
            out.append("|".join(row) + "[]")
    return "\n".join(out) + "\n"
